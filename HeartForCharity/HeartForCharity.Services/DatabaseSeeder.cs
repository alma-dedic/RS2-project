...
namespace HeartForCharity.Services
{
    public static class DatabaseSeeder
    {
        public static async Task SeedAsync(HeartForCharityDbContext context)
        {
            ...
            var addrLisa = new Address { CityId = mostar.CityId, StreetName = "Maršala Tita", Number = "47", PostalCode = "88000" };
            ...
            var globalOrg = new OrganisationProfile
            {
                ...
                AddressId = addrGlobal.AddressId,
                CreatedAt = now, UpdatedAt = now
            };
            var helpingOrg = new OrganisationProfile
            {
                ...
                OrganisationTypeId = foundationType.OrganisationTypeId,
                AddressId = addrHelping.AddressId,
                CreatedAt = now, UpdatedAt = now
            };
            var brightOrg = new OrganisationProfile
            {
                ...
                OrganisationTypeId = ngoType.OrganisationTypeId,
                AddressId = addrBright.AddressId,
                CreatedAt = now, UpdatedAt = now
            };
            ...
            var camp3 = new Campaign { OrganisationProfileId = globalOrg.OrganisationProfileId, CategoryId = catDisaster.CategoryId, Title = "Disaster Relief Ukraine", Description = "Emergency relief efforts for families displaced by conflict in Ukraine.", StartDate = now.AddMonths(-3), EndDate = now.AddMonths(6), TargetAmount = 100000, CurrentAmount = 500, Status = CampaignStatus.Active, CreatedAt = now, UpdatedAt = now };
            var camp4 = new Campaign { OrganisationProfileId = globalOrg.OrganisationProfileId, CategoryId = catEducation.CategoryId, Title = "Education for All", Description = "Building schools and providing learning materials in underserved communities.", StartDate = now.AddMonths(-1), EndDate = now.AddMonths(5), TargetAmount = 25000, CurrentAmount = 145, Status = CampaignStatus.Active, CreatedAt = now, UpdatedAt = now };
            var camp5 = new Campaign { OrganisationProfileId = helpingOrg.OrganisationProfileId, CategoryId = catFood.CategoryId, Title = "Feed the Hungry", Description = "Distributing meals and food packages to families facing food insecurity.", StartDate = now.AddMonths(-1), EndDate = now.AddMonths(3), TargetAmount = 40000, CurrentAmount = 170, Status = CampaignStatus.Active, CreatedAt = now, UpdatedAt = now };
            ...
            var camp13 = new Campaign { OrganisationProfileId = careOrg.OrganisationProfileId, CategoryId = catFood.CategoryId, Title = "Emergency Aid for Families in Need", Description = "Providing essential food parcels, hygiene kits and emergency support to families facing hardship, loss of income or displacement across the city.", StartDate = now.AddDays(-7), EndDate = now.AddMonths(5), TargetAmount = 30000, CurrentAmount = 800, Status = CampaignStatus.Active, CreatedAt = now.AddDays(-7), UpdatedAt = now.AddDays(-7) };
            ...
            await context.CampaignMedias.AddRangeAsync(
                ...
                new CampaignMedia { CampaignId = camp10.CampaignId, Url = seedImageUrl, IsCover = true, MediaType = MediaType.Image, CreatedAt = now },
                new CampaignMedia { CampaignId = camp11.CampaignId, Url = seedImageUrl, IsCover = true, MediaType = MediaType.Image, CreatedAt = now },
                new CampaignMedia { CampaignId = camp12.CampaignId, Url = seedImageUrl, IsCover = true, MediaType = MediaType.Image, CreatedAt = now },
                new CampaignMedia { CampaignId = camp13.CampaignId, Url = seedImageUrl, IsCover = true, MediaType = MediaType.Image, CreatedAt = now }
            );
            await context.SaveChangesAsync();

            
            var job1  = new VolunteerJob { OrganisationProfileId = hopeOrg.OrganisationProfileId,    CategoryId = catChildren.CategoryId,  Title = "Child Care Volunteer",        Description = "Support children in after-school activities and homework sessions.",                                                                                        StartDate = now.AddDays(7),    EndDate = now.AddMonths(3),  IsRemote = false, PositionsAvailable = 5,  PositionsFilled = 1, Status = VolunteerJobStatus.Active,    AddressId = addrHope.AddressId,    CreatedAt = now, UpdatedAt = now };
            
            var job2  = new VolunteerJob { OrganisationProfileId = hopeOrg.OrganisationProfileId,    CategoryId = catHealth.CategoryId,    Title = "Medical Camp Assistant",      Description = "Assist medical staff during free health camps for low-income communities.",                                                                              StartDate = now.AddMonths(-3), EndDate = now.AddMonths(-1), IsRemote = false, PositionsAvailable = 3,  PositionsFilled = 2, Status = VolunteerJobStatus.Completed, AddressId = addrHope.AddressId,    CreatedAt = now, UpdatedAt = now };
            
            var job3  = new VolunteerJob { OrganisationProfileId = globalOrg.OrganisationProfileId,  CategoryId = catFood.CategoryId,      Title = "Food Distribution Helper",    Description = "Help sort, pack and distribute food parcels to families in need.",                                                                                     StartDate = now.AddDays(3),    EndDate = now.AddMonths(2),  IsRemote = false, PositionsAvailable = 8,  PositionsFilled = 2, Status = VolunteerJobStatus.Active,    AddressId = addrGlobal.AddressId,  CreatedAt = now, UpdatedAt = now };
            var job4  = new VolunteerJob { OrganisationProfileId = globalOrg.OrganisationProfileId,  CategoryId = catEducation.CategoryId, Title = "English Tutor",               Description = "Teach basic English to refugee adults and children.",                                                                                                        StartDate = now.AddDays(14),   EndDate = now.AddMonths(4),  IsRemote = true,  PositionsAvailable = 4,  PositionsFilled = 0, Status = VolunteerJobStatus.Active,                                       CreatedAt = now, UpdatedAt = now };
            var job5  = new VolunteerJob { OrganisationProfileId = helpingOrg.OrganisationProfileId, CategoryId = catFood.CategoryId,      Title = "Kitchen & Meal Prep",         Description = "Prepare and serve meals at our community kitchen in Sarajevo.",                                                                                           StartDate = now.AddDays(5),    EndDate = now.AddMonths(3),  IsRemote = false, PositionsAvailable = 6,  PositionsFilled = 0, Status = VolunteerJobStatus.Active,    AddressId = addrHelping.AddressId, CreatedAt = now, UpdatedAt = now };
            var job6  = new VolunteerJob { OrganisationProfileId = helpingOrg.OrganisationProfileId, CategoryId = catHealth.CategoryId,    Title = "Health Awareness Educator",   Description = "Conduct workshops on hygiene and preventive healthcare in local schools.",                                                                              StartDate = now.AddDays(10),   EndDate = now.AddMonths(2),  IsRemote = false, PositionsAvailable = 3,  PositionsFilled = 0, Status = VolunteerJobStatus.Active,    AddressId = addrHelping.AddressId, CreatedAt = now, UpdatedAt = now };
            
            var job7  = new VolunteerJob { OrganisationProfileId = brightOrg.OrganisationProfileId,  CategoryId = catEducation.CategoryId, Title = "After-School Tutor",          Description = "Provide tutoring in math and science to students aged 10-16.",                                                                                           StartDate = now.AddMonths(-4), EndDate = now.AddMonths(-1), IsRemote = false, PositionsAvailable = 5,  PositionsFilled = 2, Status = VolunteerJobStatus.Completed, AddressId = addrBright.AddressId,  CreatedAt = now, UpdatedAt = now };
            var job8  = new VolunteerJob { OrganisationProfileId = brightOrg.OrganisationProfileId,  CategoryId = catChildren.CategoryId,  Title = "Children's Activity Leader",  Description = "Lead creative and recreational activities for children aged 5-12.",                                                                                    StartDate = now.AddDays(7),    EndDate = now.AddMonths(3),  IsRemote = false, PositionsAvailable = 4,  PositionsFilled = 0, Status = VolunteerJobStatus.Active,    AddressId = addrBright.AddressId,  CreatedAt = now, UpdatedAt = now };
            
            var job9  = new VolunteerJob { OrganisationProfileId = careOrg.OrganisationProfileId,    CategoryId = catFood.CategoryId,      Title = "Community Food Drive",        Description = "Organise and run food donation drives across the city, coordinating with local businesses and residents.",                                                StartDate = now.AddDays(1),    EndDate = now.AddMonths(2),  IsRemote = false, PositionsAvailable = 10, PositionsFilled = 3, Status = VolunteerJobStatus.Active,    AddressId = addrCare.AddressId,    CreatedAt = now, UpdatedAt = now };
            
            var job10 = new VolunteerJob { OrganisationProfileId = careOrg.OrganisationProfileId,    CategoryId = catHealth.CategoryId,    Title = "Health Screening Assistant",  Description = "Assist nurses and doctors during free community health screenings.",                                                                                    StartDate = now.AddDays(14),   EndDate = now.AddMonths(3),  IsRemote = false, PositionsAvailable = 3,  PositionsFilled = 1, Status = VolunteerJobStatus.Active,    AddressId = addrCare.AddressId,    CreatedAt = now, UpdatedAt = now };
            
            var job11 = new VolunteerJob { OrganisationProfileId = careOrg.OrganisationProfileId,    CategoryId = catChildren.CategoryId,  Title = "Winter Clothing Drive",       Description = "Collect, sort and distribute warm clothing to children and families ahead of the winter season.",                                                       StartDate = now.AddMonths(-5), EndDate = now.AddMonths(-2), IsRemote = false, PositionsAvailable = 3,  PositionsFilled = 3, Status = VolunteerJobStatus.Completed, AddressId = addrCare.AddressId,    CreatedAt = now, UpdatedAt = now };
            var jobL1 = new VolunteerJob { OrganisationProfileId = helpingOrg.OrganisationProfileId, CategoryId = catHealth.CategoryId,    Title = "Senior Care Companion Visits", Description = "Visit elderly residents at the local care home to provide social companionship and basic everyday support.",                                          StartDate = now.AddMonths(-5), EndDate = now.AddMonths(-3), IsRemote = false, PositionsAvailable = 1,  PositionsFilled = 1, Status = VolunteerJobStatus.Completed, AddressId = addrHelping.AddressId, CreatedAt = now, UpdatedAt = now };
            var jobL2 = new VolunteerJob { OrganisationProfileId = globalOrg.OrganisationProfileId,  CategoryId = catChildren.CategoryId,  Title = "Holiday Toy Drive",            Description = "Sort and wrap donated toys for distribution to children's homes during the winter holiday season.",                                                     StartDate = now.AddMonths(-6), EndDate = now.AddMonths(-4), IsRemote = false, PositionsAvailable = 1,  PositionsFilled = 1, Status = VolunteerJobStatus.Completed, AddressId = addrGlobal.AddressId,  CreatedAt = now, UpdatedAt = now };

            var job12 = new VolunteerJob { OrganisationProfileId = careOrg.OrganisationProfileId,    CategoryId = catEducation.CategoryId, Title = "Volunteer Tutor for People in Need", Description = "Tutor adults from underprivileged backgrounds learning to read and write, through flexible online sessions. Help give them the skills they need to rebuild their lives.", StartDate = now.AddDays(5),  EndDate = now.AddMonths(4),  IsRemote = true,  PositionsAvailable = 4,  PositionsFilled = 0, Status = VolunteerJobStatus.Active, CreatedAt = now, UpdatedAt = now };

            await context.VolunteerJobs.AddRangeAsync(job1, job2, job3, job4, job5, job6, job7, job8, job9, job10, job11, jobL1, jobL2, job12);
            await context.SaveChangesAsync();

            await context.VolunteerJobSkills.AddRangeAsync(
                new VolunteerJobSkill { VolunteerJobId = job1.VolunteerJobId,  SkillId = skillTeaching.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job2.VolunteerJobId,  SkillId = skillFirstAid.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job3.VolunteerJobId,  SkillId = skillFoodDist.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job3.VolunteerJobId,  SkillId = skillLogistics.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job4.VolunteerJobId,  SkillId = skillTeaching.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job5.VolunteerJobId,  SkillId = skillFoodDist.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job6.VolunteerJobId,  SkillId = skillFirstAid.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job7.VolunteerJobId,  SkillId = skillTeaching.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job8.VolunteerJobId,  SkillId = skillTeaching.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job9.VolunteerJobId,  SkillId = skillFoodDist.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job9.VolunteerJobId,  SkillId = skillLogistics.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job10.VolunteerJobId, SkillId = skillFirstAid.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job11.VolunteerJobId, SkillId = skillLogistics.SkillId },
                new VolunteerJobSkill { VolunteerJobId = jobL1.VolunteerJobId, SkillId = skillFirstAid.SkillId },
                new VolunteerJobSkill { VolunteerJobId = jobL2.VolunteerJobId, SkillId = skillTeaching.SkillId },
                new VolunteerJobSkill { VolunteerJobId = job12.VolunteerJobId, SkillId = skillTeaching.SkillId }
            );
            await context.SaveChangesAsync();
            ...
            var appL1 = new VolunteerApplication { VolunteerJobId = job1.VolunteerJobId,  UserProfileId = lisaProfile.UserProfileId, CoverLetter = "I love working with kids and would enjoy supporting after-school activities and homework help.",                Status = ApplicationStatus.Pending,   IsCompleted = false, AppliedAt = now.AddDays(-4),   UpdatedAt = now.AddDays(-4) };
            var appL2 = new VolunteerApplication { VolunteerJobId = job3.VolunteerJobId,  UserProfileId = lisaProfile.UserProfileId, CoverLetter = "I have helped sort and pack food parcels at my university's food bank.",                                         Status = ApplicationStatus.Approved,  IsCompleted = false, AppliedAt = now.AddDays(-25),  UpdatedAt = now.AddDays(-20) };
            var appL3 = new VolunteerApplication { VolunteerJobId = job5.VolunteerJobId,  UserProfileId = lisaProfile.UserProfileId, CoverLetter = "I would love to help prepare and serve meals at the community kitchen.",                                          Status = ApplicationStatus.Rejected,  RejectionReason = "Position has been filled by other applicants.", IsCompleted = false, AppliedAt = now.AddDays(-15), UpdatedAt = now.AddDays(-12) };
            var appL4 = new VolunteerApplication { VolunteerJobId = job6.VolunteerJobId,  UserProfileId = lisaProfile.UserProfileId, CoverLetter = "I am keen to share what I have learned about preventive healthcare with local schools.",                          Status = ApplicationStatus.Withdrawn, IsCompleted = false, AppliedAt = now.AddDays(-9),   UpdatedAt = now.AddDays(-7) };
            var appL5 = new VolunteerApplication { VolunteerJobId = job2.VolunteerJobId,  UserProfileId = lisaProfile.UserProfileId, CoverLetter = "I am a medical student and would value real-world experience supporting a free health camp.",                    Status = ApplicationStatus.Approved,  IsCompleted = true,  AppliedAt = now.AddMonths(-4), UpdatedAt = now.AddMonths(-1) };
            var appL6 = new VolunteerApplication { VolunteerJobId = job7.VolunteerJobId,  UserProfileId = lisaProfile.UserProfileId, CoverLetter = "I tutored secondary school students throughout my undergraduate studies in mathematics.",                        Status = ApplicationStatus.Approved,  IsCompleted = true,  AppliedAt = now.AddMonths(-4), UpdatedAt = now.AddMonths(-1) };
            var appL7 = new VolunteerApplication { VolunteerJobId = jobL1.VolunteerJobId, UserProfileId = lisaProfile.UserProfileId, CoverLetter = "I really enjoy spending time with the elderly and listening to their stories.",                                  Status = ApplicationStatus.Approved,  IsCompleted = true,  AppliedAt = now.AddMonths(-5), UpdatedAt = now.AddMonths(-3) };
            var appL8 = new VolunteerApplication { VolunteerJobId = jobL2.VolunteerJobId, UserProfileId = lisaProfile.UserProfileId, CoverLetter = "Happy to help bring some joy to children during the holidays.",                                                   Status = ApplicationStatus.Approved,  IsCompleted = true,  AppliedAt = now.AddMonths(-6), UpdatedAt = now.AddMonths(-4) };

            var appLCare = new VolunteerApplication { VolunteerJobId = job12.VolunteerJobId, UserProfileId = lisaProfile.UserProfileId, CoverLetter = "I would love to teach adult learners online — I have years of tutoring experience and flexible evening availability.", Status = ApplicationStatus.Pending, IsCompleted = false, AppliedAt = now.AddDays(-1), UpdatedAt = now.AddDays(-1) };
            ...
            await context.VolunteerApplications.AddRangeAsync(
                ...
                appL1, appL2, appL3, appL4, appL5, appL6, appL7, appL8,
                appLCare);
            await context.SaveChangesAsync();
            ...
            await context.Donations.AddRangeAsync(
                new Donation { CampaignId = camp1.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 50,  IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-30), CreatedAt = now.AddDays(-30) },
                new Donation { CampaignId = camp1.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 25,  IsAnonymous = true,  Status = DonationStatus.Success, DonationDateTime = now.AddDays(-25), CreatedAt = now.AddDays(-25) },
                new Donation { CampaignId = camp3.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 100, IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-28), CreatedAt = now.AddDays(-28) },
                new Donation { CampaignId = camp4.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 40,  IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-22), CreatedAt = now.AddDays(-22) },
                new Donation { CampaignId = camp4.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 30,  IsAnonymous = true,  Status = DonationStatus.Success, DonationDateTime = now.AddDays(-20), CreatedAt = now.AddDays(-20) },
                new Donation { CampaignId = camp5.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 75,  IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-18), CreatedAt = now.AddDays(-18) },
                new Donation { CampaignId = camp7.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 20,  IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-15), CreatedAt = now.AddDays(-15) },
                new Donation { CampaignId = camp7.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 35,  IsAnonymous = true,  Status = DonationStatus.Success, DonationDateTime = now.AddDays(-12), CreatedAt = now.AddDays(-12) },
                new Donation { CampaignId = camp8.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 60,  IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-10), CreatedAt = now.AddDays(-10) },
                new Donation { CampaignId = camp8.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 50,  IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-8),  CreatedAt = now.AddDays(-8) },
                new Donation { CampaignId = camp9.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 100, IsAnonymous = true,  Status = DonationStatus.Success, DonationDateTime = now.AddDays(-6),  CreatedAt = now.AddDays(-6) },
                new Donation { CampaignId = camp9.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 200, IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-5),  CreatedAt = now.AddDays(-5) },
                new Donation { CampaignId = camp1.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 80,  IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-3),  CreatedAt = now.AddDays(-3) },
                new Donation { CampaignId = camp3.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 150, IsAnonymous = true,  Status = DonationStatus.Success, DonationDateTime = now.AddDays(-2),  CreatedAt = now.AddDays(-2) },
                new Donation { CampaignId = camp5.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 45,  IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-1),  CreatedAt = now.AddDays(-1) }
            );

            await context.Donations.AddRangeAsync(
                new Donation { CampaignId = camp13.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 100, IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-4), CreatedAt = now.AddDays(-4) },
                new Donation { CampaignId = camp13.CampaignId, UserProfileId = lisaProfile.UserProfileId, Amount = 50,  IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-1), CreatedAt = now.AddDays(-1) },
                new Donation { CampaignId = camp13.CampaignId, UserProfileId = sarahProfile.UserProfileId, Amount = 250, IsAnonymous = false, Status = DonationStatus.Success, DonationDateTime = now.AddDays(-3), CreatedAt = now.AddDays(-3) },
                new Donation { CampaignId = camp13.CampaignId, UserProfileId = null,                       Amount = 400, IsAnonymous = true,  Status = DonationStatus.Success, DonationDateTime = now.AddDays(-2), CreatedAt = now.AddDays(-2) }
            );
            await context.SaveChangesAsync();
            ...
            await context.Reviews.AddAsync(
                new Review { VolunteerApplicationId = appL5.VolunteerApplicationId, OrganisationProfileId = hopeOrg.OrganisationProfileId, UserProfileId = lisaProfile.UserProfileId, Rating = 4, Comment = "Great experience supporting the medical camp. Well organised and the staff were patient with student volunteers — I learned a lot.", CreatedAt = now.AddMonths(-1), UpdatedAt = now.AddMonths(-1) }
            );
            await context.SaveChangesAsync();
            ...
            await context.Notifications.AddRangeAsync(
                ...
                {
                    UserProfileId          = lisaProfile.UserProfileId,
                    VolunteerApplicationId = appL2.VolunteerApplicationId,
                    Title                  = "Application Approved",
                    Message                = "Your application for 'Food Distribution Helper' has been approved. Thank you for volunteering!",
                    Type                   = NotificationType.ApplicationApproved,
                    IsRead                 = true,
                    SentDateTime           = now.AddDays(-20),
                ...
                {
                    UserProfileId          = lisaProfile.UserProfileId,
                    VolunteerApplicationId = appL3.VolunteerApplicationId,
                    Title                  = "Application Rejected",
                    Message                = "Unfortunately your application for 'Kitchen & Meal Prep' was not accepted at this time. Please check other available opportunities.",
                    Type                   = NotificationType.ApplicationRejected,
                    IsRead                 = false,
                    SentDateTime           = now.AddDays(-12),
                ...
                {
                    UserProfileId          = lisaProfile.UserProfileId,
                    VolunteerApplicationId = appL5.VolunteerApplicationId,
                    Title                  = "Application Approved",
                    Message                = "Your application for 'Medical Camp Assistant' has been approved. Thank you for volunteering!",
                    Type                   = NotificationType.ApplicationApproved,
                    IsRead                 = true,
                    SentDateTime           = now.AddMonths(-4).AddDays(2),
                    CreatedAt              = now.AddMonths(-4).AddDays(2)
                },

                new Notification
                {
                    UserProfileId          = lisaProfile.UserProfileId,
                    VolunteerApplicationId = appL6.VolunteerApplicationId,
                    Title                  = "Application Approved",
                    Message                = "Your application for 'After-School Tutor' has been approved. Thank you for volunteering!",
                    Type                   = NotificationType.ApplicationApproved,
                    IsRead                 = true,
                    SentDateTime           = now.AddMonths(-4).AddDays(2),
                    CreatedAt              = now.AddMonths(-4).AddDays(2)
                ...
                {
                    UserProfileId          = lisaProfile.UserProfileId,
                    VolunteerApplicationId = appL7.VolunteerApplicationId,
                    Title                  = "Application Approved",
                    Message                = "Your application for 'Senior Care Companion Visits' has been approved. Thank you for volunteering!",
                    Type                   = NotificationType.ApplicationApproved,
                    IsRead                 = true,
                    SentDateTime           = now.AddMonths(-5).AddDays(2),
                    CreatedAt              = now.AddMonths(-5).AddDays(2)
                },

                new Notification
                {
                    UserProfileId          = lisaProfile.UserProfileId,
                    VolunteerApplicationId = appL8.VolunteerApplicationId,
                    Title                  = "Application Approved",
                    Message                = "Your application for 'Holiday Toy Drive' has been approved. Thank you for volunteering!",
                    Type                   = NotificationType.ApplicationApproved,
                    IsRead                 = true,
                    SentDateTime           = now.AddMonths(-6).AddDays(2),
